function JDOS = JDOS_DFK(lp,sp,sc,np,Cm,Sm,selected_m)
    T = sc.delay'; 
    ...
    for m=-np.m_max:np.m_max
        ...
        JDOS = JDOS + selected_m(m+m_lkp_max+1)*prefactor.*(cos(2*m*lp.w*T)*Cm(m+m_lkp_max+1,:)+sin(2*m*lp.w*T)*Sm(m+m_lkp_max+1,:));
    end
end