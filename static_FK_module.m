function [obs_ac, obs_dc] = static_FK_module(sc,lm,lp,sp)
    ...
    %JDOS.dc = FK_JDOS(sc.XUV,sp.BG,sp.mu,0.5*lm.F); % For the DC we choose
    %cycle-averaged field strength. Turned out to give a worse fit,
    %probably due to nonlinearity. It is better to use peak field.
    JDOS.dc = FK_JDOS(sc.XUV,sp.BG,sp.mu,lm.F); % For the DC we choose cycle-averaged field strength
    ...
    obs_ac.d_JDOS = differential_profile(JDOS.ac, JDOS.zero)/cnst.c_light^3;
    ...
    obs_dc.d_JDOS = differential_profile(JDOS.dc, JDOS.zero)/cnst.c_light^3;
